...
ap.add_argument("-s", "--seconds", type=float, default = 2,
	help="path to output of access for file")
ap.add_argument("-x", "--script", type = bool, default = False,
	help="indicator if this script is being run in another script")
...
max_index = np.argmax(list(label_dict.values()))
max_value = list(label_dict.values())[max_index]
most_numerous_label = list(label_dict.keys())[max_index]
access = not (most_numerous_label == "unknown")
if not args["script"]:
